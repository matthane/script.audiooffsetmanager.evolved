...
def test_off_hits_the_all_key_as_exact(tmp_path):
    ...
    got = resolve.resolve(store, "DolbyVision", 23.976, "TrueHD", per_fps=False, distinct_spatial=True)
...
def test_off_ignores_the_fps_value_entirely(tmp_path):
    ...
    got = resolve.resolve(store, "sdr", None, "aac", per_fps=False, distinct_spatial=True)
...
def test_on_exact_hit(tmp_path):
    ...
    got = resolve.resolve(store, "dolbyvision", 23.976, "truehd", per_fps=True, distinct_spatial=True)
    assert got.entry["delay_ms"] == 175
    assert got.hit_kind == resolve.EXACT
...
def test_on_all_entries_are_dormant(tmp_path):
    ...
    store = make_store(tmp_path)
    store.set("dolbyvision|all|truehd", 100)
    got = resolve.resolve(store, "dolbyvision", 60.0, "truehd", per_fps=True, distinct_spatial=True)
    assert got.entry is None
    assert got.hit_kind == resolve.MISS
...
def test_on_fractional_rates_resolve_their_own_keys(tmp_path):
    ...
    got23 = resolve.resolve(store, "dolbyvision", 23.976, "truehd", per_fps=True, distinct_spatial=True)
    got24 = resolve.resolve(store, "dolbyvision", 24.0, "truehd", per_fps=True, distinct_spatial=True)
...
def test_flip_off_then_on_leaves_all_dormant_but_stored(tmp_path):
    store = make_store(tmp_path)
    key = resolve.write_key("dolbyvision", None, "truehd", per_fps=False, distinct_spatial=True)
    store.set(key, 100)
    assert resolve.resolve(store, "dolbyvision", 23.976, "truehd",
                           per_fps=True, distinct_spatial=True).hit_kind == resolve.MISS
    # Still in the store (management view sees it; OFF reaches it again).
    assert store.get(key)["delay_ms"] == 100
    assert resolve.resolve(store, "dolbyvision", 23.976, "truehd",
                           per_fps=False, distinct_spatial=True).entry["delay_ms"] == 100


def test_flip_on_then_off_leaves_specific_dormant_but_stored(tmp_path):
    store = make_store(tmp_path)
    key = resolve.write_key("dolbyvision", 23.976, "truehd", per_fps=True, distinct_spatial=True)
    store.set(key, 175)
    assert resolve.resolve(store, "dolbyvision", 23.976, "truehd",
                           per_fps=False, distinct_spatial=True).hit_kind == resolve.MISS
    # Still in the store (management view sees it; ON reaches it again).
    assert store.get(key)["delay_ms"] == 175
    assert resolve.resolve(store, "dolbyvision", 23.976, "truehd",
                           per_fps=True, distinct_spatial=True).entry["delay_ms"] == 175
...
def test_write_key_off_targets_the_all_key(tmp_path):
    assert resolve.write_key("DolbyVision", 23.976, "TrueHD",
                             per_fps=False, distinct_spatial=True) == "dolbyvision|all|truehd"


def test_write_key_on_targets_the_specific_key(tmp_path):
    assert resolve.write_key("DolbyVision", 23.976, "TrueHD",
                             per_fps=True, distinct_spatial=True) == "dolbyvision|23|truehd"
...
def test_write_then_resolve_roundtrips_as_exact(tmp_path):
    ...
    store = make_store(tmp_path)
    store.set(resolve.write_key("dv", None, "truehd", per_fps=False, distinct_spatial=True), 175)

    at60 = resolve.resolve(store, "dv", 60.0, "truehd", per_fps=True, distinct_spatial=True)
    assert (at60.hit_kind, at60.entry) == (resolve.MISS, None)

    store.set(resolve.write_key("dv", 60.0, "truehd", per_fps=True, distinct_spatial=True), 120)
    at60 = resolve.resolve(store, "dv", 60.0, "truehd", per_fps=True, distinct_spatial=True)
    assert (at60.hit_kind, at60.entry["delay_ms"]) == (resolve.EXACT, 120)
    at24 = resolve.resolve(store, "dv", 24.0, "truehd", per_fps=True, distinct_spatial=True)
...
def test_unheard_of_formats_resolve_like_any_other(tmp_path):
    store = make_store(tmp_path)
    key = resolve.write_key("HDR10+", 119.88, "x-future-codec", per_fps=True, distinct_spatial=True)
    assert key == "hdr10plus|119|x-future-codec"
    store.set(key, -115)
    got = resolve.resolve(store, "HDR10+", 119.88, "x-future-codec", per_fps=True, distinct_spatial=True)
...
def test_legacy_spelled_file_entries_resolve_after_load(tmp_path):
    ...
    store.load()
    got = resolve.resolve(store, "HDR10+", 23.976, "TrueHD", per_fps=False, distinct_spatial=True)
...
def test_on_with_unparseable_fps_consults_the_all_key(tmp_path):
    ...
    store = make_store(tmp_path)
    store.set("sdr|all|aac", -50)
    got = resolve.resolve(store, "sdr", "abc", "aac", per_fps=True, distinct_spatial=True)
    assert got.entry["delay_ms"] == -50
    assert got.hit_kind == resolve.EXACT
...
def test_on_with_unparseable_fps_and_empty_store_is_a_miss(tmp_path):
    store = make_store(tmp_path)
    got = resolve.resolve(store, "sdr", None, "aac", per_fps=True, distinct_spatial=True)
...
def test_write_key_stays_strict_on_unparseable_fps(tmp_path):
    ...
    with pytest.raises(ValueError):
        resolve.write_key("sdr", "abc", "aac", per_fps=True, distinct_spatial=True)
...
def test_resolution_ms_accessor_keeps_entry_shape_internal(tmp_path):
    ...
    hit = resolve.resolve(store, "sdr", None, "aac", per_fps=False, distinct_spatial=True)
    assert hit.ms == -115
    miss = resolve.resolve(store, "sdr", None, "flac", per_fps=False, distinct_spatial=True)
...
def test_on_marked_miss_carries_the_specific_key_only(tmp_path):
    ...
    store.delete("dolbyvision|all|truehd")
    got = resolve.resolve(store, "dolbyvision", 23.976, "truehd",
                          per_fps=True, distinct_spatial=True)
    assert got.hit_kind == resolve.MISS
...
def test_unmarked_resolutions_carry_no_reset_keys(tmp_path):
    ...
    miss = resolve.resolve(store, "hdr10", 24.0, "ac3", per_fps=False, distinct_spatial=True)
...
def test_spatial_off_consults_the_base_key(tmp_path):
    ...
    assert (got.hit_kind, got.key, got.ms) == (
        resolve.EXACT, "dolbyvision|all|truehd", 175)
    assert got.tried == ("dolbyvision|all|truehd",)
...
def test_spatial_off_never_reads_the_variant_entry(tmp_path):
    ...
    store = make_store(tmp_path)
    store.set("dolbyvision|all|truehd_atmos", -125)
    got = resolve.resolve(store, "dolbyvision", 23.976, "truehd_atmos",
                          per_fps=False, distinct_spatial=False)
...
def test_spatial_on_never_reads_the_base_entry_for_a_variant(tmp_path):
    ...
    got = resolve.resolve(store, "dolbyvision", 23.976, "truehd_atmos",
                          per_fps=False, distinct_spatial=True)
    assert (got.entry, got.hit_kind) == (None, resolve.MISS)
    assert got.tried == ("dolbyvision|all|truehd_atmos",)
...
def test_spatial_off_composes_with_per_fps_on(tmp_path):
    store = make_store(tmp_path)
    store.set("dolbyvision|23|truehd", -50)
    got = resolve.resolve(store, "dolbyvision", 23.976, "truehd_atmos",
                          per_fps=True, distinct_spatial=False)
    assert (got.hit_kind, got.key) == (resolve.EXACT, "dolbyvision|23|truehd")
